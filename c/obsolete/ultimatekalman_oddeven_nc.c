...
#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, length, 1000, assign_indices);
#else
	assign_indices(kalman, indices, length, 0, length);
#endif
...
void parallel_smooth(kalman_t* kalman, step_t* *indices, int length) {
	...
	#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, (length + 1)/2,blocksize,G_F_to_R_tilde);
	#else
	G_F_to_R_tilde(kalman, indices, length, 0, (length + 1)/2);
	#endif

	//Second part of the algorithm
	#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, (length + 1)/2,blocksize,H_R_tilde_to_R);
	#else
	H_R_tilde_to_R(kalman, indices, length, 0, (length + 1)/2);
	#endif

	//Third part of the algorithm
	#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, length/2,blocksize,H_tilde_G_to_G_tilde);
	#else
	H_tilde_G_to_G_tilde(kalman, indices, length, 0, length/2);
	#endif

	//Fix the last index

	// int lmo = indices[length - 2];
	...
	#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, length/2, blocksize, Variables_Renaming);
	#else
	Variables_Renaming(kalman, indices, length, 0, length/2);
	#endif
	...
	#ifdef PARALLEL
	parallel_for_c(new_indices, indices, length, length/2,blocksize, Init_new_indices);
	#else
	Init_new_indices(new_indices, indices, length, 0, length/2);
	#endif
	...
	#ifdef PARALLEL
	parallel_for_c(kalman, indices, length, (length + 1)/2, blocksize, Solve_Estimates);
	#else
	Solve_Estimates(kalman, indices, length, 0, (length + 1)/2);
	#endif
